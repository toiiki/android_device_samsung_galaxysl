                                  ...
                                  OMX_STATETYPE DesiredState)
{
	...
	if ( eError != OMX_ErrorNone ) {
		LOGE("Error: Couldn't get state for component or sent to invalid state because of an error.\n");
		return eError;
	}
                           ...
                           OMX_U32 nData1, OMX_U32 data2, OMX_PTR pEventData)
{
    ...
    if ( eError != OMX_ErrorNone ) {
        LOGE("Error: From JPEGENC_GetState\n");
    }
    switch ( eEvent ) {
        ...
        case OMX_EventError:
                if (nData1 == OMX_ErrorHardware){
                    ...
                    bPreempted = 1;
                    LOGE("APP:: OMX_ErrorResourcesPreempted !\n\n");
                }
        ...
        case OMX_EventBufferFlag:
            LOGE("Event Buffer Flag detected\n");
        case OMX_EventMax:
        case OMX_EventMark:
            break;
            
        default:
                LOGE("ErrorNotification received: Error Num %p: String :%s\n", (OMX_PTR)nData1, (OMX_STRING)pEventData);
    }
...
int fill_data (OMX_BUFFERHEADERTYPE *pBuf, void * inputBuffer, int buffSize)
{
    ...
    LOGE ("Buffer Size = %d. Read %d bytes from buffer. \n", (int) nRead, (int)nRead);
...
int encodeImage(void* outputBuffer, void *inputBuffer, int width, int height, int quality)
{
	...
	if ( error != OMX_ErrorNone ) {
	    LOGE("Error returned by OMX_Init()\n");
	    goto EXIT;
	}

	LOGD("OMX_Init Successful!\n");
	...
	if ( error != OMX_ErrorNone ) {
	    LOGE("%d::APP_Error at function call: %x\n", __LINE__, error);        
	...
	if ( error != OMX_ErrorNone ) {
	    LOGE ("Error from SendCommand-Idle(Init) State function\n");
	    goto EXIT;
	}
	...
	LOGD("Waiting for OMX_StateIdle\n");
	error = WaitForState(pHandle, OMX_StateIdle);
	if ( error != OMX_ErrorNone ) {
	    LOGE("Error: JpegEncoder->WaitForState has timed out or failed - Idle %X\n",
	          error);
	    goto EXIT;
	}
	...
	if ( error != OMX_ErrorNone ) {
	    LOGE (stderr,"Error from SendCommand-Executing State function\n");
	    goto EXIT;
	}
	pComponent = (OMX_COMPONENTTYPE *)pHandle;
    
	LOGD("Waiting for OMX_StateExcecuting\n");
	error = WaitForState(pHandle, OMX_StateExecuting);
	if ( error != OMX_ErrorNone ) {
	    LOGE("Error:  JpegEncoder->WaitForState has timed out or failed  - Executing %X\n", error);
	    goto EXIT;
	}
	...
	while ((error == OMX_ErrorNone) && 
	        ...
	        if (bPreempted)
	        {
			LOGD("Preempted - Forced tp Idle State - Waiting for OMX_StateIdle\n");
			error = WaitForState(pHandle, OMX_StateIdle);
			if ( error != OMX_ErrorNone ) {
				LOGE("Error:  JpegEncoder->WaitForState has timed out or failed %X",  error);
				goto EXIT;
			}
			break;
	        }
		...
		sigaddset(&set,SIGALRM);
	        ...
	        if ( FD_ISSET(Event_Pipe[0], &rfds)) {
				...
				if(bError) {
					...
					if (error != OMX_ErrorNone) {
						LOGE("APP:: Error:  JpegEncoder->WaitForState has timed out or failed %X\n", error);
						goto EXIT;
					}
					LOGE("APP:: Component is in Invalid state now.\n");
					goto EXIT;	
				}
				break;

	                case OMX_EventBufferFlag:
		                LOGE("APP:: EOS flag received\n");
		                break;

	                default:
		                LOGE("APP:: Non-error event rised. Event -> 0x%x\n", EventPrivate.eEvent);
		                break;
	            }
	        }
		...
		if ( FD_ISSET(IpBuf_Pipe[0], &rfds) && !DEINIT_FLAG ) {
			...
			if (nRepeated >= maxRepeat) {                    
				...
				LOGD("Sending another output buffer\n");
	        ...
	        if (DEINIT_FLAG == 1) {
                    ...
                    error = WaitForState(pHandle, OMX_StateIdle);
                    if ( error != OMX_ErrorNone ) {
                        LOGE("Error:  JpegEncoder->WaitForState has timed out %X", error);
                        goto EXIT;
                    }

                    error = OMX_SendCommand(pHandle, OMX_CommandPortDisable, 0, NULL);
                    if ( error != OMX_ErrorNone ) {
                        LOGE("Error from SendCommand-PortDisable function\n");
                        goto EXIT;
                    }
	        }
	        ...
	        if (done == 1) {
                    ...
                    if ( error != OMX_ErrorNone ){
                        LOGE("Warning:  JpegEncoder->JPEGENC_GetState has returned status %X\n", error);
                        goto EXIT;
                    }
	        }
		LOGD("JPEG Encoding Complete. Freeing OMX Handle.\n");
	}
EXIT:
	FREE(pPortParamType);
	FREE(pQfactorType);
	...
	if( error != OMX_ErrorNone){
		...
		for (nCounter = 0; nCounter < NUM_OF_BUFFERSJPEG; nCounter++) {
		    ...
		    if ( (error != OMX_ErrorNone) ) {
		        LOGE ("Error in OMX_FreeBuffer: %d\n", __LINE__);
		    }
	...
	error = TIOMX_Deinit();
	if ( error != OMX_ErrorNone ) {
	    LOGE("Error returned by OMX_DeInit()\n");
	}
    return fileSize;
}
